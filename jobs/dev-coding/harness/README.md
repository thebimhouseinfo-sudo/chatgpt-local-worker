...
- `planning-bundle-check.mjs --dir <planning-dir> [--task-id TASK-001]` — verify the standard planning bundle (`ARCHITECTURE.md`, `IMPLEMENTATION_PLAN.md`, `TODO.md`, `TASKS.md`), lint task-ledger IDs/statuses, and optionally confirm the selected task exists.
...
- `completion-gate.mjs --cwd <repo> [--planning-dir <dir>] [--task-id TASK-001]` — aggregate structural gates, quality discovery, and optional final planning-bundle/task-ledger validation. Add `--run-quality` only after reviewing discovered commands.
- `validate.mjs` — validate the Dev Coding Job Pack itself, including planning-bundle/task-ledger/task-subplan capabilities.

Normal ordering is: read planning bundle → select/update task → inspect only implementation-relevant code → implement/validate → update `TASKS.md` → run bundle-aware completion gate. A bounded new branch may get a task-local plan; project-level replanning belongs to `dev-planing`.
