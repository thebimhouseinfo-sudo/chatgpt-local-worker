...
| TASK-V2-013 | IN_PROGRESS | Windows resident tray host + per-user auto-start + thin idle lifecycle | TASK-V2-008, TASK-V2-009 | logon starts hidden resident host+tunnel; tray controls status/guide/restart/exit; no daily launcher; idle keeps no execution resources solely for readiness; ChatGPT call wakes execution | Source implementation added in gptworker-tray.ps1 + setup/run integration; awaiting live Windows acceptance |
...
- Windows resident tray host source implementation is now present: single-instance tray, hidden Worker/Tunnel supervision, per-user Run-key auto-start, guide/restart/exit, low-frequency health refresh and source logs under AppData. Live Windows acceptance is still required.
