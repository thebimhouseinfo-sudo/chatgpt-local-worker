...
**Implementation task list:** [DC-001–DC-050](./dev-coding-task-list.md) — executable tasks, dependencies, status and acceptance evidence.
...
**Meaningful negative controls and known limitation:** Where pre-fix red-green is unavailable, seed a behaviorally relevant implementation/fixture defect in an isolated run (e.g. wrong API response, wrong grading result, disabled expected navigation) mapped to the named acceptance signal. Keep the **same test unchanged** and record normal PASS → faulty behavior FAIL on the *relevant assertion* → restored PASS, along with test/source hashes, mutation description and exit codes. A test modified to always throw, an unrelated defect or forced runner failure is **not** valid evidence. **Known limitation:** even a meaningful negative control cannot prove exhaustive test coverage. If a material acceptance signal remains untested, mark coverage PARTIAL/UNVERIFIED and require applicable independent runtime/browser observation or human review instead of claiming comprehensive verification.
...
### Checkpoint discovery and safe resume after a chat reset

After Workspace confirmation and `dev-coding` Job activation, the **existing preflight harness** must perform a bounded, read-only scan of `.gptworker/dev-coding/*/state.json` within the canonical Workspace; no remembered task ID is required. Only valid, unfinished checkpoints are candidates; skip completed, corrupt, out-of-Workspace or symlink-escaped entries. Return sanitized metadata: task ID, non-sensitive goal summary, last result, remaining gap, update time and fingerprint freshness. One matching unfinished task ⇒ offer **Resume or start new**; multiple plausible tasks ⇒ show a short list and require explicit task selection; a user-supplied task ID takes precedence over recency. Never auto-resume or execute the checkpoint's `next_action`.

Upon **user-authorized resume**, re-read schema/version, canonical Workspace and task contract, then recompute actual file/test/config fingerprints before trusting any evidence. Chat-reset creates a new work handle and generation: rebind validated task state to the newly confirmed execution, **never reuse the old authority token**. If source inputs drifted, retain prior history but invalidate affected PASS evidence and require relevant checks/replanning. Scanning cannot launch browsers, mutate files, activate a previous Job or create an independent task registry. Extend `harness/execution-preflight.mjs` with a minimal safe checkpoint helper, not a second orchestration system.
