...
const packRoot = path.join(repoRoot, "jobs", "dev-planning");
...
assert.equal(validation.id, "dev-planning");
assert.equal(validation.status, "ready");
assert.equal(validation.dev_planning.skills, 5);
assert.equal(validation.dev_planning.source_editing, false);
...
try {
  ...
  const concrete = template
    .replace("Describe the observable engineering objective and success condition.", "Add deterministic handoff between development planning and coding execution.")
    .replace("List concrete repository findings that constrain the plan. Prefer file paths, symbols, schemas, tests, commands, and current behavior.", "jobs/dev-planning/job.yaml defines the planning pack. jobs/coding/job.yaml accepts an optional plan input. scripts/test-job-runtime.mjs validates runtime activation boundaries.")
    .replace("List explicit technical, compatibility, platform, dependency, delivery, and user constraints.", "Planning must not edit source code. Coding must not create a formal plan. Job Runtime confirmation and pack isolation remain unchanged.")
    ...
    .replace("1. Describe an ordered, implementation-ready change step with affected files/components/symbols and expected behavior.\n2. Continue in dependency order.", "1. Update dev-planning pack metadata/SOP/skills and validate its plan-only boundary.\n2. Remove formal planning from coding metadata/SOP while retaining optional plan consumption.\n3. Update runtime and harness tests for both ready jobs.\n4. Run Job Pack validation and the inherited full test suite.")
    .replace("List targeted tests/checks first, then broader build/runtime/migration verification appropriate to the risk.", "Run dev-planning plan lint, coding harness tests, Job Runtime lifecycle tests, TypeScript build, and the inherited npm test suite.")
    ...
    .replace("Record any execution ordering, migration, rollout, environment, or delivery notes the coding job must preserve.", "The coding job may consume this plan as an optional input but must follow newer explicit user instructions and repository rules if they conflict.");
...
console.log("test-dev-planning-harness: ok");
