...
assert.ok(MCP_QUICKSTART.includes("Valid ACTIVE evidence is exactly one thing"));
assert.ok(MCP_QUICKSTART.includes("NOT activation evidence without"));
assert.ok(MCP_QUICKSTART.includes("If task + Workspace are present but @gptworker is absent, do not call GPTWorker at all"));
