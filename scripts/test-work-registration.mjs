...
assert.equal(await comparablePath(validated.workspace), await comparablePath(wsA));
...
assert.deepEqual(
  new Set(await Promise.all(observed.map(comparablePath))),
  new Set(await Promise.all([wsA, wsB].map(comparablePath)))
);
