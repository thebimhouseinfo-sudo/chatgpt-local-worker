...
- The only acceptable FOLDER is a path that the user personally provides during the current PREPARE window.
- Once the user has explicitly supplied FOLDER in the current PREPARE window, GPT may keep using that same user-supplied FOLDER while the user changes JOB or refines TASK.
- GPT MUST NOT replace that user-supplied FOLDER with another path unless the user supplies a different folder.
- If the user has not yet supplied FOLDER in the current PREPARE window, GPT MUST ask: "Cho tôi đường dẫn tới thư mục làm việc."
...
2. Infer only JOB. Collect TASK from the user. Accept FOLDER only when the user explicitly supplies it during the current PREPARE window.
...
export function buildServerInstructions(
  ...
  const controlSurface = [
    ...
    "GPTWorker plugin/@ invocation opens a NEW PREPARE window. Call job_list exactly once with surface=welcome. If the invocation is bare, return welcome_text verbatim. If the same user message also contains a task, Job hint, or folder, DO NOT force the generic Welcome; read that content immediately. GPT may infer only JOB. GPT MUST NEVER nominate, infer, suggest, autocomplete, restore, or reuse a FOLDER that the user has not supplied. Once the user explicitly supplies FOLDER during the current PREPARE window, that same user-supplied FOLDER may be retained while the user changes JOB or refines TASK. Never replace it with another path unless the user supplies a different folder. Once JOB + user-supplied FOLDER + TASK are known, call job_select confirmed=false immediately; never ask the user to invoke GPTWorker again.",
  ...
  const header = [
    ...
    "The startup cwd is not project authority and must never be used as a candidate FOLDER. GPT may infer JOB only. FOLDER must come from the user during the current PREPARE window.",
